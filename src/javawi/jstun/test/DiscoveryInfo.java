...
public class DiscoveryInfo {
	...
	public DiscoveryInfo(InetAddress testIP, String stunServer) {
		this.testIP = testIP;
	...
	public String toString() {
		...
		sb.append("Tested Network interface: ");
		try {
			sb.append(NetworkInterface.getByInetAddress(testIP).getName());
		} catch (SocketException se) {
			sb.append("unknown");
		}
		sb.append(" to " + stunServer);
		sb.append("\n");
		sb.append("Local IP address: ");
		sb.append(testIP.getHostAddress() + " : " + localPort);
		sb.append("\n");
		if (error) {
		...
		if (publicIP != null) {
			sb.append(publicIP.getHostAddress() + " : " + publicPort);
		} else {
			sb.append("unknown");
		}
		sb.append("\n");
