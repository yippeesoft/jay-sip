...
public class SipMessageHandler {
	...
	/**
	 * @return the current AudioSipFormats
	 */
	public ArrayList<SipAudioFormat> getSipAudioFormats(){
		return mSipAudioFormats;
	}
	/**
	 * @return the Vector audioformats with given sdp
	 * @throws SdpException 
	 */
	public Vector getSipAudioFormats(SessionDescription sdp) throws SdpException
	{
		MediaDescriptionImpl mimply = (MediaDescriptionImpl)sdp.getMediaDescriptions(true).get(0);
		return mimply.getMediaField().getFormats();
	}
	/**
	 * 
	 * @param callercodec
	 * @return the vector of audio codec,sdp negotiation between caller and callee
	 */
	public Vector<String> getSipAudioFormatsBump(Vector<String> callercodec){
		int len=callercodec.size();
		int temp = 0;
		Vector<String>  m_temp = (Vector<String> ) callercodec.clone();
		Vector local_m = new Vector();
		for (SipAudioFormat audioFormat : localSipProfile.getAudioFormats()) {
			local_m.add(audioFormat.getFormat());
		}
		for(int i=0;i<len;i++){
			temp=Integer.parseInt(callercodec.get(i).toString());
			if(local_m.contains(temp)==false)
				m_temp.remove(callercodec.get(i));
		}
		return m_temp;
	}
	
	public ArrayList<SipAudioFormat> getSipAudioFormatsList(Vector<String> audioformats)
	{
		int format_type;
		ArrayList<SipAudioFormat> audioList = new ArrayList<SipAudioFormat>();
		for(int i=0;i<audioformats.size();i++){
			 format_type=Integer.parseInt(audioformats.get(i).toString());
			 SipAudioFormat sa = new SipAudioFormat(format_type, SdpConstants.avpTypeNames[format_type], SdpConstants.avpClockRates[format_type]);
			 audioList.add(sa);
		}
		return audioList;
	}
	...
	public SessionDescription createSDP(Request request) throws SdpException {
		...
		if (request == null) {
			...
			//peter cao  added 
			audioFormats=getSipAudioFormats(sdpSession);
			audioFormats=getSipAudioFormatsBump(audioFormats);
			if(audioFormats.size()==0)
				return null;
			mSipAudioFormats = new ArrayList<SipAudioFormat>();
			//set the new audio formats to the mSipAudioFormats.
		    mSipAudioFormats = getSipAudioFormatsList(audioFormats);
			
		    for(SipAudioFormat item:mSipAudioFormats)
			{
					AttributeField attributeField = new AttributeField();
					attributeField.setName(SdpConstants.RTPMAP);
					attributeField.setValue(item.getSdpField());
					audioDescription.addAttribute(attributeField);
			}
		}
